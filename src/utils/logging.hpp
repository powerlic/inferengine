 ...
 */
#ifndef TUNI_LOGGING_H_
#define TUNI_LOGGING_H_
...
#define CHECK_BINARY_OP(name, op, x, y)                                 \
  if (easycv::LogCheckError _check_err = easycv::LogCheck##name(x, y))    \
    easycv::LogMessageFatal(__FILENAME__, __LINE__).stream()                 \
...
#define CHECK_NOTNULL(x) \
  ((x) == NULL ? easycv::LogMessageFatal(__FILENAME__, __LINE__).stream() << "Check  notnull: "  #x << ' ', (x) : (x)) // NOLINT(*)
// Debug-only checking.
#ifdef NDEBUG
#define DCHECK(x) \
  while (false) CHECK(x)
...
#define LOG_INFO easycv::LogMessage(__FILENAME__, __LINE__)

#define LOG_ERROR LOG_INFO
#define LOG_WARNING LOG_INFO
#define LOG_FATAL easycv::LogMessageFatal(__FILENAME__, __LINE__)
...
#define LOG_IF(severity, condition) \
  !(condition) ? (void)0 : easycv::LogMessageVoidify() & LOG(severity)
