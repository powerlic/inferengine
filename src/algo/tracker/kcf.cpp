...
void KCF::Init(cv::Mat &image, cv::Rect rect_init) {
  ...
  // cout<<window_sz_<<endl;
  float output_sigma = std::sqrt(float(target_sz_.area())) * output_sigma_factor_ / cell_size_;
  // cout<<output_sigma<<endl;
  cv::Mat labels = GaussianShapedLabels(output_sigma, FloorSizeScale(window_sz_, 1. / cell_size_));
  
  cv::dft(labels, yf_, cv::DFT_COMPLEX_OUTPUT);
  // cout<<yf_.rows<<"x"<<yf_.cols<<"x"<<yf_.channels()<<endl;
  // cout<<yf_<<endl;

  cos_window_ = CalculateHann(yf_.size());
  // cout<<cos_window_<<endl;
...
cv::Rect KCF::Update(cv::Mat &image) {
  ...
  for (unsigned int i = 0; i < z.size(); ++i)
    cv::dft(z[i], zf_vector[i], cv::DFT_COMPLEX_OUTPUT);
  ...
  // cout<<"kzf"<<endl;
  // cout<<kzf(cv::Rect(0, 0, 5, 5))<<endl;
  // cv::Mat tmp = ComplexMul(model_alphaf_, kzf);
  // cout<<"tmp"<<tmp(cv::Rect(0, 0, 5, 5))<<endl;
  cv::Mat response;
  cv::idft(ComplexMul(model_alphaf_, kzf), response, cv::DFT_SCALE | cv::DFT_REAL_OUTPUT); // Applying IDFT
  // cout<<"response"<<response(cv::Rect(0, 0, 5, 5))<<endl;
  cv::Point maxLoc;
...
void KCF::Learn(cv::Mat &patch, float lr) {
  ...
  // cout<<kf(cv::Rect(0,0,5,5))<<endl;
  cv::Mat alphaf = ComplexDiv(yf_, kf + cv::Scalar(lambda_, 0));
  // cout<<alphaf(cv::Rect(0,0,5,5))<<endl;
...
cv::Mat KCF::GaussianShapedLabels(float sigma, cv::Size sz) {
  ...
  // cout<<labels(cv::Rect(0,0,15,15))<<endl;
...
cv::Mat KCF::GaussianCorrelation(std::vector<cv::Mat> &xf, std::vector<cv::Mat> &yf) {
  ...
  cv::Mat xy(xf[0].size(), CV_32FC1, cv::Scalar(0.0)), xyf, xy_temp;
  for (unsigned int i = 0; i < xf.size(); ++i) {
...
cv::Mat KCF::PolynomialCorrelation(std::vector<cv::Mat> &xf, std::vector<cv::Mat> &yf) {
  std::vector<cv::Mat> xyf_vector(xf.size());
  cv::Mat xy(xf[0].size(), CV_32FC1, cv::Scalar(0)), xyf, xy_temp;
  for (unsigned int i = 0; i < xf.size(); ++i) {
    cv::mulSpectrums(xf[i], yf[i], xyf, 0, true);
    cv::idft(xyf, xy_temp, cv::DFT_SCALE | cv::DFT_REAL_OUTPUT); // Applying IDFT
    xy += xy_temp;
...
cv::Mat KCF::LinearCorrelation(std::vector<cv::Mat> &xf, std::vector<cv::Mat> &yf) {
  ...
  for (unsigned int i = 0; i < xf.size(); ++i) {
    // cout<<xf[i](cv::Rect(0,0,2,2))<<endl;
    // cout<<yf[i](cv::Rect(0,0,2,2))<<endl;
    cv::mulSpectrums(xf[i], yf[i], xyf, 0, true);
    
    kf += xyf;
  }
  // cout<<"kf:"<<kf<<endl;
  // cout<<kf(cv::Rect(0,0,2,2))<<endl;
  float numel_xf = xf[0].size().area() * xf.size();
  // cout<<"numel_xf:"<<numel_xf<<endl;
  // cout<<kf(cv::Rect(0,0,2,2))/numel_xf<<endl;
  return kf/numel_xf;
}
