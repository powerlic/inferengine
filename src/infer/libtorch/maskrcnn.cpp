...
MaskRcnn::MaskRcnn(int device_id):m_device(torch::kCUDA, device_id)
{
    m_cell_anchors = 3;
    input_size_ = cv::Size(1248, 800);
    nms_layer_ = NmsLayer();
}

void MaskRcnn::Init(const std::string& model_prefix)
{
    string base_model = model_prefix + "/mask_rcnn_backbone.pt";
    //string rpn_model = model_prefix + "/rpn.pt";
    //string classify = model_prefix + "/classify.pt";
    // baseFeaturePath
    ...
    string box_head_model = model_prefix + "/mask_rcnn_boxhead.pt";
    m_boxhead = torch::jit::load(box_head_model);
    m_boxhead.to(m_device);
    m_boxhead.eval();

    // classifyPath
    // m_classify = torch::jit::load(classify);
    // m_classify.to(m_device);
    // m_classify.eval();

    string mask_head_model = model_prefix + "/mask_rcnn_maskhead.pt";
    m_mask = torch::jit::load(mask_head_model);
    m_mask.to(m_device);
    m_mask.eval();
...
void MaskRcnn::generate_cell_anchors()
{   
...
torch::Tensor MaskRcnn::bbox_transform_torch(torch::Tensor& anchors, torch::Tensor& deltas)
{
...
torch::Tensor MaskRcnn::bbox_transfrom_torch2(torch::Tensor& rois, torch::Tensor& deltas)
{
...
void MaskRcnn::generate_all_anchors(std::vector<cv::Size> feature_map_sizes)
{
    // p2-p6 anchors
...
torch::Tensor MaskRcnn::mlvl_nms(const torch::Tensor& deltas, const torch::Tensor& scores)
{
...
torch::Tensor MaskRcnn::target_lvls(torch::Tensor& rois)
{
...
void MaskRcnn::Detect(const cv::Mat& image, std::vector<DetectedObject>& detected_objects)
{
    ...
    cv::Size debug_input_size(1333, 750);
    cv::resize(bgr_img, show, debug_input_size);
    ...
    //stage3 mask refine
    auto mask_levels = target_lvls(final_preds);
    std::vector<torch::Tensor> stage3_mask_features;
    std::vector<torch::Tensor> stage3_inds;
    for(long i=0; i <level_count_; i++)
    {
        auto inds = torch::nonzero(mask_levels == i).squeeze(1);
        stage3_inds.push_back(inds);
        if(inds.size(0) > 0) {
            auto level_proposals = final_preds.index_select(0, inds);
            auto x1 = level_proposals.select(1, 0).clamp(0, input_size_.width-1).view({-1,1});
            auto y1 = level_proposals.select(1, 1).clamp(0, input_size_.height-1).view({-1,1});
            auto x2 = level_proposals.select(1, 2).clamp(0, input_size_.width-1).view({-1,1});
            auto y2 = level_proposals.select(1, 3).clamp(0, input_size_.height-1).view({-1,1});
            auto label = level_proposals.select(1, 4).view({-1,1});
            level_proposals = torch::cat({label, x1, y1, x2, y2}, 1);
            torch::Tensor level_mask_feature = torch::zeros({level_proposals.size(0), 256, 14, 14}).toType(torch::kFloat).to(m_device);
            if(i==0) {
                ROIAlign_forwardV1(p2_feature, level_proposals, 14, 14, 0.25, 2, level_mask_feature);
            }
            else if(i==1){
                ROIAlign_forwardV1(p3_feature, level_proposals, 14, 14, 0.125, 2, level_mask_feature);
            }
            else if(i==2){
                ROIAlign_forwardV1(p4_feature, level_proposals, 14, 14,  0.0625, 2, level_mask_feature);
            }
            else if(i==3){
                ROIAlign_forwardV1(p5_feature, level_proposals, 14, 14, 0.03125, 2, level_mask_feature);
            }
            stage3_mask_features.push_back(level_mask_feature);
        }
    }
    torch::Tensor mask_features = torch::cat({stage3_mask_features}, 0);
    torch::Tensor stage3_rerange_inds = torch::cat({stage3_inds}, 0);
    mask_features = mask_features.index_select(0, stage3_rerange_inds);
    torch::Tensor mask_probs = m_mask.forward({mask_features}).toTensor();
    // cout<<"mask_probs"<<endl;
    // cout<<mask_probs<<endl;
    // parse mask

    final_preds = final_preds.to(torch::kCPU);
    assert(mask_probs.size(0)==final_preds.size(0));
    ...
    for(auto i=0; i<final_preds.size(0);i++)
    {
        ...
        float c_y = (y1 + y2)/2;
        //pasrse mask
        torch::Tensor instance_mask_probs = mask_probs.select(0, i).select(0, label).to(torch::kCPU);
        cv::Mat mask(raw_mask_size_, raw_mask_size_, CV_32FC1, static_cast<void*>(instance_mask_probs.data<float>()));
        if(x2>x1 && y2>y1) 
        {
            detected_objects.push_back(DetectedObject(c_x, c_y, w, h, label, score, mask));
        }
    }
